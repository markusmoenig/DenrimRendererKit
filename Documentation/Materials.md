...
* `Examples/SceneScripts/MaterialVariants/material-variants.denrim` provides the same idea as a script template with reusable material includes and relative mesh paths.
...
Applications can author rectangular area lights with `QuadLight` and `RenderScene.addQuadLight(_:)`. This is a convenience over emissive quad meshes; it does not create default lights in an empty scene.
